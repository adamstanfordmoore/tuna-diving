...
import piecewise_linear_regression
...
class Tag(object):
    ...
    def addVelocityColumn(self):
        ...
        for i in range(0, len(dives)):
            ...
            dive["velocity"] = pd.ewma(pd.Series(velocity , index=dive.index), 10)
            dives[i] = dive
        print("finished")
    ...
    def getThermocline(self):
        depthChannelIndex = self.findIndex("depth")
        tempChannelIndex = self.findIndex("temperature")
        dives = self.dives
        for dive in dives:
            x = dive.iloc[:, depthChannelIndex]
            y = dive.iloc[:, tempChannelIndex]
            print(x.shape)
            print(y.shape)
            model = piecewise_linear_regression.MixedLayerRegression()
            model.fit(x, y)
            print(model.psi)
